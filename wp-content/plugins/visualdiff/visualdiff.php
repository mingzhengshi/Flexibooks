...
// duplicate post
//add_action( 'admin_action_rd_duplicate_post_as_draft', 'fb_duplicate_post_as_draft' );
//add_filter( 'post_row_actions', 'fb_duplicate_post_link', 10, 2 );
...
function fb_duplicate_post_as_draft(){
	global $wpdb;
	if (! ( isset( $_GET['post']) || isset( $_POST['post'])  || ( isset($_REQUEST['action']) && 'fb_duplicate_post_as_draft' == $_REQUEST['action'] ) ) ) {
 ...
 */
function fb_duplicate_post_link( $actions, $post ) {
	if (current_user_can('edit_posts')) {
		$actions['duplicate'] = '<a href="admin.php?action=fb_duplicate_post_as_draft&amp;post=' . $post->ID . '" title="Duplicate this document" rel="permalink">Duplicate</a>';
	}
	return $actions;
}
